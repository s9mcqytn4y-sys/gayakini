...
class InventoryService(
    ...
    @Transactional
    fun reserveStock(orderId: UUID, orderItemId: UUID, variantId: UUID, quantity: Int): InventoryReservation {
        val variant = variantRepository.findWithLockById(variantId)
    ...
    @Transactional
    fun releaseReservation(orderItemId: UUID, reason: String) {
        val reservation = reservationRepository.findByOrderItemId(orderItemId)
            .orElseThrow { NoSuchElementException("Reservasi tidak ditemukan.") }

        if (reservation.status != ReservationStatus.ACTIVE) return

        val variant = variantRepository.findWithLockById(reservation.variant.id)
            .orElseThrow { NoSuchElementException("Varian produk tidak ditemukan.") }
